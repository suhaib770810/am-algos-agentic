...
const fixedAllocRecs = {
    ...
    processEachPosition: (positionRow, allocationFloor, allocationCeiling) => {
        try {
            ...
            let targetNumberOfContractsForFloor = Math.max(Math.ceil(allocationFloor / averagePositionValue), 1);
            // Calculate the number of contracts that would correspond to the allocation ceiling.
            // Round up rather than round down
            let targetNumberOfContractsForCeiling = Math.max(Math.ceil(allocationCeiling / averagePositionValue), 1);
            ...
            if (targetNumberOfContractsForFloor === targetNumberOfContractsForCeiling) {
                targetNumberOfContracts = targetNumberOfContractsForFloor;
                recNumber = targetNumberOfContracts - position;
                if (recNumber > 0) {
            ...
            return {
                ...
                currentRecommendation: {
                    ...
                    targetPositionValue: Number((targetNumberOfContracts * averagePositionValue).toFixed(2)),
                    recNumber,
                    recAtCurrentPrice,
